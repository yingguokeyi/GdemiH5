var hour = [{ "name":'1小时', "value":'1' },{ "name":'2小时', "value":'2' },{ "name":'3小时', "value":'3' }];
//借用上展的json转换器
;(function(){
    var data = [];
	hour = JSON.stringify(hour).replace(/\"id\":/g, "\"value\":");
	hour=JSON.parse(hour);
	// console.log(hour);
	for (var i = 0,length = hour.length; i < length; i++) {
            data.push(hour[i]);
    };
 ...
 * 一级
 */
var method3=$('.select-value2').mPicker({
    level:1,
    dataJson:dataJson,
    Linkage:false,
    rows:6,
